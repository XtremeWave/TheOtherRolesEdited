    ...
    class MeetingHudPatch {
        ...
        static void swapperConfirm(MeetingHud __instance) {
            ...
            if (firstPlayer != null && secondPlayer != null) {
                ...
                Swapper.charges--;
                meetingExtraButtonText.text = $"剩余换票数: {Swapper.charges}";
            }
        }
        ...
        static void populateButtonsPostfix(MeetingHud __instance) {
            ...
            if (addSwapperButtons || addMayorButton) {
                ...
                meetingExtraButtonText.text = addSwapperButtons ? $"剩余换票数: {Swapper.charges}" : "";
                ...
                if (Portalmaker.portalmaker != null && (PlayerControl.LocalPlayer == Portalmaker.portalmaker || Helpers.shouldShowGhostInfo()) && !Portalmaker.portalmaker.Data.IsDead) {
                    if (Portal.teleportedPlayers.Count > 0) {
                        ...
                        foreach (var entry in Portal.teleportedPlayers) {
                            ...
                            msg += Portalmaker.logShowsTime ? $"{(int)timeBeforeMeeting}秒之前: " : "";
                            msg = msg + $"{entry.name} 使用了传送装置\n";
                        }
                        ...
                        FastDestroyableSingleton<HudManager>.Instance.StartCoroutine(Effects.Lerp(0.4f, new Action<float>((x) => {
                            if (x == 1f) {
                                foreach (PlayerControl p in PlayerControl.AllPlayerControls) {
                                    ...
                                    output += "- " + RoleInfo.GetRolesString(p, false, false, true) + ", 最后一次露面" + roomName + "\n";
                                }
            ...
            public class MeetingHudUpdatePatch
            {
                ...
                static void Postfix(MeetingHud __instance)
                {
                    ...
                    if (Blackmailer.blackmailer != null && Blackmailer.blackmailed != null)
                    {
                        ...
                        var playerState = __instance.playerStates.FirstOrDefault(x => x.PlayerId == Blackmailer.blackmailed.PlayerId);
